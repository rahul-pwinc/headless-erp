...
# ---------------------------------------------------------------------------
# Idempotency. Every integration retries, and a retry after a deadlock without
# this creates a second invoice. The key is stored on the invoice in a Custom
# Field with unique=1, so the DATABASE enforces the race rather than this
# script: two simultaneous first-attempts cannot both succeed, and the loser
# fails rather than double-posting.
#
# A replay returns the original document and creates nothing. It is reported as
# replayed:true so a caller can tell "your write landed" from "your write landed
# just now", which matters when the first response was lost.
# Hoisted above the replay branch: the replay path reads AUDIT_DOCTYPE to
# report which audit records already exist for the original document.
# Left inside the else branch this NameErrors on every replay.
...
REPLAYED = None
if IDEMPOTENCY_KEY:
    REPLAYED = frappe.db.get_value("Sales Invoice",
                                   {"intent_idempotency_key": IDEMPOTENCY_KEY}, "name")

if REPLAYED:
    # Replay. Return the original and create nothing: no document, no audit
    # record, no side effect. Reported as replayed:true so a caller can tell
    # "your write landed" from "your write landed just now", which is the
    # distinction that matters when the first response was lost.
    d0 = frappe.get_doc("Sales Invoice", REPLAYED)
    logs = frappe.get_all(AUDIT_DOCTYPE, filters={"target_name": REPLAYED},
                          fields=["name"])
    frappe.response["message"] = {
        "name": d0.name, "grand_total": d0.grand_total, "lines": [],
        "drift_detected": 0, "audit_records": [x["name"] for x in logs],
        "audit_error": None, "replayed": True,
    }
