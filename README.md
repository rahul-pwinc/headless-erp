...
**The 3 corpus failures are deliberate.** `srv-01`, `srv-02`, `srv-03` use a raw path that bypasses the contract on purpose, to document what ERPNext does natively. Each names the `derivation_map.md` finding it documents. They stay failing because they are findings, not defects to tune away.
