...
| accounting corpus | **49 / 54** across 8 categories | `reports/clean/corpus.json` |
...
| concurrency | integrity held; 3/20 writes landed, 3 audit records, 0 audit errors | `reports/clean/concurrency.json` |
| trial balance | guaranteed balanced by construction, therefore uninformative | `reports/clean/trial_balance.json` |
...
**The 5 corpus failures are deliberate.** `srv-01`, `srv-02`, `srv-03`, `srv-05`, `srv-06` use a raw path that bypasses the contract on purpose, to document what ERPNext does natively. Each names the `derivation_map.md` finding it documents. They stay failing because they are findings, not defects to tune away.

<!-- END GENERATED NUMBERS -->

Everything in `reports/clean/` comes from one `make all` run against a site
created for the purpose, from a clean clone. Not from the long-lived instance
